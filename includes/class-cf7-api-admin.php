...
class QS_CF7_api_admin{
    ...
    public function verify_dependencies(){
        if( ! is_plugin_active('contact-form-7/wp-contact-form-7.php') ){
            $notice = array(
                'id'                  => 'cf7-not-active',
                'type'                => 'warning',
                'notice'              => __( 'Você precisa instalar o plugin CONTACT FORM 7 para que a integração funcione.' ,$this->textdomain ),
    ...
    function add_integrations_tab($panels){
        ...
        $integration_panel = array(
            'title'    => __( 'API Leads Jetimob' , $this->textdomain ),
    ...
    function wpcf7_integrations( $post ) {
		...
		$json_placeholder = __('*** SUBSTITUA OS CAMPOS PELOS QUE CORRESPONDEM AOS DO SEU FORMUÁRIO **
			{ "full_name": "[nome]", "email": "[email]", "country_code": "55", "phone": "12355678", "message": "[mensagem]", "subject": "[assunto]", "source": "Formulário do Site" }' , '');
        ?>


        <h2><?php echo esc_html( __( 'Configurar Integração', $this->textdomain ) ); ?></h2>
        ...
        <fieldset>
            ...
            <div class="cf7_row">
                ...
                <label for="wpcf7-sf-send_to_api">
                    ...
                    <?php _e( 'Enviar leads pela API?' , $this->textdomain );?>
                </label>
 ...
                    <?php _e( 'URL de Integração' , $this->textdomain );?>
 ...
                    <?php _e( 'Chave de Autorização (Authorization-key)' , $this->textdomain );?>
        ...
        <fieldset data-qsindex="params">
			<div class="cf7_row">
				<h2><?php echo esc_html( __( 'Campos do formulário', $this->textdomain ) ); ?></h2>

	            <table>
	                <tr>
	                    <th><?php _e( 'Campos do formulário' , $this->textdomain );?></th>
	                    <th><?php _e( 'Chave de API' , $this->textdomain );?></th>
						<th></th>
	                </tr>
		...
		<fieldset data-qsindex="json">
			<div class="cf7_row">
				<h2><?php echo esc_html( __( 'Template JSON', $this->textdomain ) ); ?></h2>

				<legend>
					<span>Tags do formulário:</span>
					<?php foreach( $mail_tags as $mail_tag) : ?>
						<?php if( $mail_tag->type == 'checkbox'):?>
							<?php foreach( $mail_tag->values as $checkbox_row ):?>
				...
				<legend>
					<span>Cole o texto a seguir no campo abaixo e faça as substituições necessrias para que os valores correspondam aos campos do seu formulário:</span><br />
					{ "full_name": "[nome]", "email": "[email]", "country_code": "55", "phone": "12355678", "message": "[mensagem]", "subject": "[assunto]", "source": "Formulário do Site" }
				</legend>
		...
		<fieldset>
			<div class="cf7_row">
				<label class="debug-log-trigger">
					+ <?php _e( 'DEBUG LOG ( Ver última tentativa de envio )' , $this->textdomain); ?>
				</label>
				<div class="debug-log-wrap">
					<h3 class="debug_log_title"><?php _e( 'ÚLTIMA CHAMADA DE API' , $this->textdomain );?></h3>
					<div class="debug_log">
						<h4><?php _e( 'Url chamada' , $this->textdomain );?>:</h4>
						<textarea rows="1"><?php echo trim(esc_attr( $debug_url ));?></textarea>
						<h4><?php _e( 'Parâmetros' , $this->textdomain );?>:</h4>
